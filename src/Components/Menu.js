...
import { CHANGE_WALLET, SELECT_MENU, PORTFOLIO_DATA, FOOTNOTES_DATA } from "../actionTypes";
import CONFIG from "./../config.dev.json";
import axios from "axios";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import runtimeEnv from "@mars/heroku-js-runtime-env";
import { Button, FormControl, Select, MenuItem, InputLabel, Box } from "@mui/material";
...
const Menu = ({ openMenu, updateMenu, allowedMenu }) => {
  ...
  const handleCancelProgress = useCallback(async () => {
    axios.post(
      `${SERVE_URL}/cancel/${walletAddress}${walletChain ? "/" + walletChain : ""}${walletProtocol ? "/" + walletProtocol : ""}` //TODO: Multi chain
    );
        ...
        interval = setInterval(async () => {
          try {
            ...
            if (response.progress) {
              ...
              setProgressVal(response.progress/* > 99 ? 99 : response.progress*/);
      ...
      {isLoading ? (
        ...
        >
          ...
          >
            ...
            <Button onClick={handleCancelProgress} variant="contained" color="error" sx={{ mt: 2 }}>
              {t('CancelBtnText')}
            </Button>
          ...
          <ul className="navbar-nav">
            <Link className="nav-link" to="/wallet">
              ...
              >
                ...
                <h6 className="menu-item-text">{t('Wallet')}</h6>
              </li>
            </Link>
            <Link
              className="nav-link"
            ...
            >
              ...
              >
                ...
                <h6 className="menu-item-text">{t('Portfolio')}</h6>
              </li>
            </Link>
            <Link className="nav-link" to="/help">
              ...
              >
                ...
                <h6 className="menu-item-text">{t('Help')}</h6>
              </li>
            </Link>
            <Box sx={{ mt: 2 }}>
              <li
                ...
                <FormControl fullWidth>
                  ...
                  >
                    <MenuItem value={"en"}>
                      <ReactCountryFlag countryCode="US" svg style={{ marginRight: '10px' }} />
                      English
                    </MenuItem>
                    <MenuItem value={"es"}>
                      <ReactCountryFlag countryCode="ES" svg style={{ marginRight: '10px' }} />
                      Español"
                    </MenuItem>
                  </Select>
                </FormControl>
