...
const Portfolio = ({ data, updateMenu, openMenu, allowedMenu, allowMenu }) => {
    ...
    const [valuePopupAnchorEl, setValuePopupAnchorEl] = useState(null);
    const openValuePopup = Boolean(valuePopupAnchorEl);
    ...
    const handleClickValue = (index, event) => {
        setValuePopupAnchorEl(event.currentTarget);
    ...
    useEffect(() => {
        ...
        setportfolioData(portfolioPreData.map(item => {
            ...
            return {
                ...
                supply_token_list: item?.supply_token_list,
                reward_token_list: item?.reward_token_list
        ...
        <div className="main-board">
            ...
            <div className="main-board-title-div d-flex flex-column flex-lg-row">
                ...
                <div className="main-board-title-current-value-div">
                    {getTotalVal.totalValueStr.length < 12 ? (<span>{t('CurValText')}</span>) : <></>}
                ...
                <div className="main-board-title-filter-div " onClick={() => onFilter()} data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                    <div className="d-flex main-board-title-filter-btn">
                        <h6 className="main-board-title-filer-title">{t('FilterText')}</h6>
            ...
            <div className="filter-button-group row collapse" id="collapseExample">
                <Box sx={{ minWidth: 120 }}>
                    <FormControl fullWidth>
                        <InputLabel id="demo-simple-select-label">{t('FilterType')}</InputLabel>
                        ...
                        >
                            <MenuItem onClick={() => onFilterType('all')} value={'All'}>{t('FilterAll')}</MenuItem>
                ...
                <Box sx={{ minWidth: 120 }}>
                    <FormControl fullWidth>
                        <InputLabel id="demo-simple-select-label">{t('FilterChain')}</InputLabel>
                        <Select style={{ borderColor: '#5e5e5e' }}
                            labelId="demo-simple-select-label"
                            id="demo-simple-select"
                            value={chain}
                        ...
                        >
                            <MenuItem onClick={() => onFilterChain('all')} value={'All'}>{t('FilterAll')}</MenuItem>
                ...
                <table className="table portfolio-table">
                    <thead>
                        <tr className="table-header-tr">
                            ...
                            {portfolio_table2.map((item, index) => {
                                const tooltipString = { cost: t('CostTooltip'), value: t('CurValText') };
                                if (item.header === "cost" || item.header === "value") {
                                    return (
                                        <Tooltip key={index} title={tooltipString[item.header]} arrow enterDelay={200} leaveDelay={200}>
                                            <th className="table-header  right-area" onClick={() => onSort_2(index)}>{t("TableH_" + item.header)}
                                ...
                                return <th key={index} className="table-header  right-area" onClick={() => onSort_2(index)}>{t("TableH_" + item.header)}
                                    <img className="sort-image" src={`${process.env.PUBLIC_URL}${index !== selHeader2 ? '/assets/images/sort_both.png' : sort_state ? '/assets/images/sort_top.png' : '/assets/images/sort_bottom.png'}`} alt="" />
                                </th>
                            })}
                        </tr>
                    ...
                    <tbody>
                        {(() => {
                            ...
                            return portfolio_table_body_data.map((item, index) => {
                                ...
                                return <><tr key={index} className="accordion-toggle table-tr"
                                    ...
                                    <td className="table-body-num vertical-align-middle">
                                        <div className="table-data-value" onClick={(e) => handleClickValue(index, e)}>
                                            {item.value_str}
                                        </div>
                                    </td>
                                        ...
                                        <td colSpan="8" className="collapes-td">
                                            <div id={item.id + item.chain} className="collapse collapse1">
                                                <div className="row border-1">
                                                    ...
                                                    <div className="col-3 sub-table-header left-area">{t('HisTableH_DATE')}</div>
                                                    <div className="col-3 sub-table-header left-area">{t('HisTableH_TOKEN')}</div>
                                                    <div className="col-2 sub-table-header right-area">{t('HisTableH_UNITS')}</div>
                                                    <div className="col-2 sub-table-header right-area">{t('HisTableH_COST')}</div>
                                                    <div className="col-2 sub-table-header right-area">{t('HisTableH_FEE')}</div>
                                                </div>
                        ...
                        {portfolio_table_body_data.length > 0 ?
                            <tr className=" total-tr">
                                <td></td>
                                <td colSpan="3" className="table-total-title">{t('Table_TotalText')}</td>
                            ...
                            : <tr className=" total-tr total-tr-no-data"><td colSpan="8">{t('NoDataText')}</td></tr>}
                    </tbody>
                </table>
            ...
            <Popover
                open={openValuePopup}
                onClose={() => setValuePopupAnchorEl(null)}
                anchorEl={valuePopupAnchorEl}
                anchorOrigin={{
                    vertical: "center",
                    horizontal: "right"
                }}
            ...
            >
                {
                    openValuePopup === true && (() => {
                        const item = portfolio_table_body_data[openValueIndex];
                        if (item == undefined) {
                            return;
                        }
                        console.log(item);
                        return (
                            ...
                            >
                                ...
                                {item.supply_token_list?.length > 0 ?
                                    <Grid
                                        sx={{ mr: 5 }}
                                        direction="column"
                                        justifyContent="center"
                                    ...
                                    >
                                        {
                                            item.supply_token_list?.map((token) => {
                                                console.log(token);
                                                return (
                                                    <div key={`supply-${item.id}-${token.symbol}`}>
                                                        {`${numberWithCommas(token.amount.toFixed(2))} ${token.symbol}`}
                                                    </div>
                                                )
                                            })
                                        }
                                    ...
                                    <Grid
                                        ...
                                        alignItems="flex-start"
                                    >
                                        <div key={`assets-${item.id}-${item.assets[0].ticker}`}>
                                            {`${numberWithCommas(item.units.toFixed(2))} ${item.assets[0].ticker}`}
                                        </div>
                                    </Grid>
                                }
                                ...
                                {item.reward_token_list &&
                                    ...
                                    >
                                        {
                                            item.reward_token_list?.map((token) => {
                                                console.log(token);
                                                return (
                                                    <div key={`supply-${item.id}-${token.symbol}`}>
                                                        {`${numberWithCommas(token.amount.toFixed(2))} ${token.symbol} ($${numberWithCommas((token.price * token.amount).toFixed(0))})`}
                                                    </div>
                                                )
                                            })
                                        }
